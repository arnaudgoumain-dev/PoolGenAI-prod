...
const TRANSLATIONS = {
  fr: {
    ...
    note_anti_phos: "Traitement curatif si phosphates > 100 ppb. Si toujours trop élevé après 24h, ajouter 50 mL/10 m³ en complément. En entretien préventif régulier (indépendant de la mesure) : 20 mL/10 m³. Verser devant la buse de refoulement, filtration en marche 24h.",
    note_sequestrant: "Traitement cuivre/fer, corrosif : manipuler avec précaution. Un excédent diminue la dureté (TH) de l'eau — respecter la dose. Utiliser de préférence avec un pH entre 7 et 7,5. Verser en périphérie du bassin, filtration en marche.",
  ...
  en: {
    ...
    note_anti_phos: "Curative treatment if phosphates > 100 ppb. If still too high after 24h, add 50 mL/10 m³ more. For regular preventive maintenance (regardless of reading): 20 mL/10 m³. Pour in front of return jet, filtration running 24h.",
    note_sequestrant: "Copper/iron treatment, corrosive: handle with care. Excess amounts lower water hardness (TH) — respect the dose. Best used with pH between 7 and 7.5. Pour around pool perimeter, filtration running.",
  ...
  de: {
    ...
    note_anti_phos: "Kurbehandlung bei Phosphaten > 100 ppb. Falls nach 24h noch zu hoch, 50 mL/10 m³ zusätzlich hinzufügen. Für regelmäßige vorbeugende Pflege (unabhängig vom Messwert): 20 mL/10 m³. Vor der Rückflussdüse eingießen, Filtration 24h laufen lassen.",
    note_sequestrant: "Kupfer/Eisen-Behandlung, ätzend: mit Vorsicht handhaben. Eine Überdosierung senkt die Wasserhärte (TH) — Dosierung einhalten. Am besten bei einem pH-Wert zwischen 7 und 7,5 anwenden. Am Beckenrand eingießen, Filtration läuft.",
  ...
  it: {
    ...
    note_anti_phos: "Trattamento curativo se fosfati > 100 ppb. Se dopo 24h è ancora troppo alto, aggiungere altri 50 mL/10 m³. Per manutenzione preventiva regolare (indipendente dalla misurazione): 20 mL/10 m³. Versare davanti all'ugello di ritorno, filtrazione 24h.",
    note_sequestrant: "Trattamento rame/ferro, corrosivo: maneggiare con cura. Un eccesso riduce la durezza (TH) dell'acqua — rispettare la dose. Da usare preferibilmente con pH tra 7 e 7,5. Versare intorno alla vasca, filtrazione in funzione.",
  ...
  es: {
    ...
    note_anti_phos: "Tratamiento curativo si los fosfatos superan 100 ppb. Si sigue alto tras 24h, añadir 50 mL/10 m³ más. Para mantenimiento preventivo regular (independiente de la medición): 20 mL/10 m³. Verter delante del inyector de retorno, filtración 24h.",
    note_sequestrant: "Tratamiento cobre/hierro, corrosivo: manipular con cuidado. Un exceso reduce la dureza (TH) del agua — respeta la dosis. Se recomienda usar con pH entre 7 y 7,5. Verter alrededor de la piscina, filtración en marcha.",
  ...
  pt: {
    ...
    note_anti_phos: "Tratamento curativo se os fosfatos ultrapassarem 100 ppb. Se continuar alto após 24h, adiciona mais 50 mL/10 m³. Para manutenção preventiva regular (independente da medição): 20 mL/10 m³. Despejar na frente do injetor de retorno, filtração 24h.",
    note_sequestrant: "Tratamento cobre/ferro, corrosivo: manusear com cuidado. Um excesso reduz a dureza (TH) da água — respeita a dose. Usar preferencialmente com pH entre 7 e 7,5. Despejar ao redor da piscina, filtração em funcionamento.",
...
const DEFAULT_PRODUCTS = [
  ...
  },
  {
    ...
    action: "ph+",
    // v1.95.0 — Fix : valeur confirmée par étiquette produit réelle
    // (100g/10m³ pour +0,1 pH), l'ancienne valeur de 20g était 5x trop basse.
    doseAmount: 100,
    doseUnit: "g",
    effectAmount: 0.1,
    effectPer: 10,
  ...
  },
  {
    ...
    action: "chlore",
    // v1.95.0 — Fix : 150g était une erreur de config (facteur x10). Un
    // hypochlorite de calcium non stabilisé est actif à ~65-70%, ce qui
    // donne ~15g/10m³/1mg/L, pas 150g. Confirmé par un cas réel (breakpoint
    // chlore combiné) : 15g/10m³ donnait 2,05kg pour 72m³/delta 19mg/L,
    // cohérent avec les fourchettes usuelles (2,0-2,1kg pour ce type de
    // produit), alors que 150g donnait 20,52kg (10x trop).
    doseAmount: 15,
    doseUnit: "g",
    effectAmount: 1,
    effectPer: 10,
  ...
  // courant 5 L.
  {
    ...
    action: "o2",
    // v1.95.0 — Fix : 70mL était une erreur de config (facteur x10). Le
    // commentaire de v1.75.0 ci-dessus indique lui-même une cible de
    // "6 à 8 cL/m³" (60-80 mL/m³) pour 10mg/L, soit 600-800 mL pour 10m³ —
    // pas 70 mL. 700 mL = milieu de la fourchette documentée.
    doseAmount: 700,
    doseUnit: "mL",
    effectAmount: 10,
    effectPer: 10,
  ...
  // calibre différemment de son effet sur le pH (cf. reco_note_tac_minus).
  {
    ...
    action: "tac-",
    // v1.95.0 — Fix : valeur confirmée par étiquette produit réelle
    // (250g/10m³ pour -10mg/L de TAC). L'ancienne valeur de 200g était une
    // copie non calibrée du produit TAC+ (bicarbonate), chimiquement
    // différent — voir commentaire v1.56.0 ci-dessus qui l'annonçait déjà.
    doseAmount: 250,
    doseUnit: "g",
    effectAmount: 10,
    effectPer: 10,
  ...
  },
  {
    ...
    action: "phos-",
    // v1.95.0 — Fix : valeur confirmée par fiche produit Mareva Stop-Phosphate
    // (dose curative 100mL/10m³ quand le taux dépasse 100 ppb, avec rappel de
    // 50mL/10m³ si toujours trop élevé après 24h — voir note_anti_phos).
    // Dose préventive d'entretien (indépendante de la mesure) : 20mL/10m³,
    // mentionnée dans la note mais pas automatisée ici (pas de suivi
    // périodique de type maintenanceRatio pour ce produit).
    doseAmount: 100,
    doseUnit: "mL",
    effectAmount: 100,
    effectPer: 10,
  ...
  },
  {
    ...
    action: "sequestrant",
    // v1.95.0 — Fix : valeur confirmée par fiche technique Mareva Stop-Métal
    // ("0,1L désactive environ 2mg/L de métal pour 10m³" = 50mL/10m³/1mg/L).
    // L'ancienne valeur de 100mL était 2x trop haute — un surdosage réel de
    // ce produit corrosif fait aussi baisser la dureté (TH) de l'eau.
    doseAmount: 50,
    doseUnit: "mL",
    effectAmount: 1,
    effectPer: 10,
